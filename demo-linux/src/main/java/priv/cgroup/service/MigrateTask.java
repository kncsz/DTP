...
public class MigrateTask {
    ...
    public Map<String, Object> migrateTask(Map<String, Object> requestBody) {
        ...
        try {
            ...
            String parentCgroupPath = path.get("cgroupPath") == null ? "" : path.get("cgroupPath");
            ...
            if (migrateResult == 0) {
                ...
                if (!Objects.equals(currentPath, "")) {
                    ...
                    try {
                        ...
                        removeTaskFromConfigFile(configPath, pid);
    ...
    private Map<String, String> getPath(int parentHierarchy,String parentName) {
        Cgroup selectedCgroup = cgroupRepository.selectCgroup(parentHierarchy, parentName);
    ...
    private void addTaskToConfigFile(String configFilePath, String pid) throws IOException {
        ...
        // 如果 tasks 节点存在，直接设置 pid
        ObjectNode tasksNode;
        if (configContent.has("tasks")) {
            tasksNode = (ObjectNode) configContent.get("tasks");
        } else {
            tasksNode = objectMapper.createObjectNode();
            configContent.set("tasks", tasksNode);
        }

        tasksNode.put(pid, "");  // 添加 PID 作为键值对

        objectMapper.writerWithDefaultPrettyPrinter().writeValue(configFile, configContent);
    }

    private void removeTaskFromConfigFile(String configFilePath, String pid) throws IOException {
        File configFile = new File(configFilePath);
        if (!configFile.exists()) {
            return;
        }
